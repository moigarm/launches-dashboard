// Funding data from free sources: Google News RSS + Y Combinator directory
...
async function fetchGoogleNewsFunding(query: string): Promise<FundingEvent[]> {
	try {
		...
		for (const item of items) {
			...
			const companyMatch = title.match(/^([^,]+?)\s+(?:raises?|secures?|closes?|lands?|gets?|bags?|pulls?)/i);
			if (!companyMatch) continue;

			const companyName = companyMatch[1].trim();

			// Extract dollar amount from headline, e.g. "$5M", "$12 million", "£3m"
			const amountMatch = title.match(/[\$£€](\d+(?:\.\d+)?)\s*(B|M|K|billion|million|thousand)?/i);
...
async function fetchYCFunding(batches: string[] = ['W25', 'S24', 'W24']): Promise<FundingEvent[]> {
	const events: FundingEvent[] = [];
		...
		try {
			...
			const response = await fetch('https://45bwzj1sgc-dsn.algolia.net/1/indexes/*/queries', {
				...
				headers: {
					...
					'x-algolia-api-key': 'Zjk5ZmU5OTU4ZmExMGMxNTNhZTdhZmFiOGY4NDk2MTI5NzJjNWQxNzg1YTlhNTU0YTMxMzliMjBlNTdlYzE0NWZpbHRlcnM9JnJlc3RyaWN0SW5kaWNlcz15Y19jb21wYW55XzE='
				},
				body: JSON.stringify({
					requests: [
						{
...
// Note: OpenVC was evaluated but requires investor registration (no public API).
...
async function fetchTechCrunchFunding(): Promise<FundingEvent[]> {
	try {
		...
		for (const item of items) {
			...
			if (!/(raise[sd]?|fund(s|ing|ed)|round|invest|secures?|closes?)/i.test(title)) continue;

			const companyMatch = title.match(/^([^,]+?)\s+(?:raises?|secures?|closes?|lands?|gets?)/i);
			if (!companyMatch) continue;
...
export async function searchFundingEvents(): Promise<FundingEvent[]> {
	const [googleNews, ycFunding, techCrunch] = await Promise.all([
	...
	for (const event of [...googleNews, ...ycFunding, ...techCrunch]) {
...
export async function getCompanyFunding(companyName: string) {
